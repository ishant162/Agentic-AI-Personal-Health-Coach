...
def llm_inference_decision(user_input: str, llm) -> InferenceDecision:
    ...
    - "Positive": Direct LLM response (for general health questions,
    no symptoms, wellness inquiries, or low-risk scenarios)
    - "Negative": EHR data retrieval (for symptom reporting, specific
    health concerns, or cases requiring medical history)
    ...
    Determine whether this requires EHR data access or can be handled with
    a direct response.
    ...
    try:
        ...
        print(
            f"LLM Decision: {response.decision} "
...
def fallback_decision(user_input: str) -> InferenceDecision:
    ...
    has_symptoms = any(
        keyword in user_input.lower() for keyword in symptoms_keywords
    )
    ...
    return InferenceDecision(
        ...
        confidence=0.6,
        reasoning="""
        No clear symptoms detected using keyword matching (fallback)
        """,
    )
